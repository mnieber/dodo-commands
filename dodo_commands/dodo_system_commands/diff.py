...
if Dodo.is_main(__name__):
    ...
    default_project_path = os.path.join(project_dir, "dodo_commands",
                                        "default_project")
    ...
    if args.defaults_dir:
        if os.path.exists(default_project_path):
            raise CommandError(
                ...
                "but the symlink %s already exists" % default_project_path)

        target_dir = os.path.join(project_dir, args.defaults_dir)
        if not os.path.exists(target_dir):
            raise CommandError(
                "The --default-dir option was supplied, " +
                "but the target directory %s does not exist" % target_dir)

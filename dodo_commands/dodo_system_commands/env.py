...
def _args():
    ...
    parser.add_argument('--create-python-env', action="store_true")
...
class Activator:
    ...
    def init(self, env, use_latest):
        ...
        if use_latest:
            if env:
                self._report(
                    "Options --latest and <env> are mutually exclusive\n")
                return False
            env = self.latest_env
            if not env:
    ...
    def run(self, init, create, create_python_env, forget):
        if create:
            project_dir = os.path.expanduser(
                os.path.join(
                    self.global_config.get("settings", "projects_dir"),
                    self.env))
        ...
        with undo(undo_steps):
            python_env_dir = os.path.join(
                project_dir, ".env") if create_python_env else None
            if False:
                alt_python_env_dir = os.path.join(project_dir, "dodo_commands",
                                                  "env")
                python_env_dir = alt_python_env_dir or alt_python_env_dir
            if create or init:
                if os.path.exists(self.env_dir):
                    self._report("Cannot create an environment because the " +
                ...
                register_env(self.env, project_dir, config_dir, python_env_dir,
                             undo_steps)

            if create_python_env:
                register_python_env(
                    self.global_config.get("settings", "python_interpreter"),
                    self.env, python_env_dir, undo_steps)
        ...
        activate_filename = os.path.join(self.env_dir,
                                         "activate.%s" % self.shell)
...
if Dodo.is_main(__name__, safe=False):
    ...
    if activator.init(args.env, args.use_latest):
        activator.run(args.init, args.create, args.create_python_env,
                      args.forget)
